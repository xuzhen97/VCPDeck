...
describe("jobs command", () => {
    it("未知子命令返回用法，--help 零开销输出用法", async () => {
        ...
        expect(lines.filter((line) => line.includes("vcpdeck jobs list"))).toHaveLength(2);
    });
    it("拒绝非法 status、page 和未知选项", async () => {
        const { port } = await listen(() => { });
        const { paths, processEnv } = await fixture(port);
        await expect(runJobsCommand("list", ["--status=nope"], { paths, processEnv })).rejects.toThrow("--status");
        await expect(runJobsCommand("list", ["--page=0"], { paths, processEnv })).rejects.toThrow("--page");
        await expect(runJobsCommand("list", ["--watch"], { paths, processEnv })).rejects.toThrow("未知选项");
    });
    it("list 输出分页表格；--client 按名称解析为 clientId", async () => {
        ...
        expect(requests.some((url) => url.startsWith("/api/jobs?clientId=client-1"))).toBe(true);
    ...
    it("--client 未匹配到机器时明确报错", async () => {
        ...
        const { paths, processEnv } = await fixture(port);
        await expect(runJobsCommand("list", ["--client=ghost"], { paths, processEnv })).rejects.toThrow('未找到 Client "ghost"');
    });
    ...
    describe("jobs run", () => {
        it("缺少机器或命令 token 时返回用法错误", async () => {
            await expect(runJobsCommand("run", ["workstation"])).rejects.toThrow("Jobs 命令");
        });
        it("拒绝非法 timeout", async () => {
            const { port } = await listen(() => { });
            const { paths, processEnv } = await fixture(port);
            await expect(runJobsCommand("run", ["--timeout=abc", "--", "echo", "hi"], {
                paths,
                processEnv,
        ...
        it("创建 exec Job 并把命令 token 以空格连接为 command 模式", async () => {
            ...
            const { port } = await listen((request, response) => {
                if ((request.url ?? "") === "/api/clients") {
                    response.setHeader("content-type", "application/json");
                    response.end(JSON.stringify([{ clientId: "client-1", name: "ws", online: true }]));
                    return;
                }
            ...
            const { paths, processEnv } = await fixture(port);
            const lines = [];
            await runJobsCommand("run", ["ws", "--cwd=D:\\tmp", "--timeout=60", "--", "git", "status"], { paths, processEnv, log: (message) => lines.push(message) });
        ...
        it("仅在多个 token 的空白参数边界会丢失时警告", async () => {
            const { port } = await listen((request, response) => {
                ...
                response.end(JSON.stringify({ jobId: "job-run-1", status: "running", type: "exec" }));
            });
            const { paths, processEnv } = await fixture(port);
            const warnings = [];
            const context = {
        ...
        it("--wait --json 的 stdout 为单一 JSON，状态提示只写 stderr", async () => {
            ...
            const { port } = await listen((request, response) => {
                ...
                if (url === "/api/jobs" && request.method === "POST") {
                    created++;
                    response.end(JSON.stringify({ jobId: `job-json-${created}`, status: "running", type: "exec" }));
                    return;
                }
            ...
            for (const shouldFail of [false, true]) {
                const stdout = [];
                const stderr = [];
                const promise = runJobsCommand("run", ["ws", "--wait", "--json", "--", "echo", "a b"], {
                    paths,
                    processEnv,
                    pollIntervalMs: 1,
                ...
                if (shouldFail)
                    await expect(promise).rejects.toThrow("终态为 error");
                else
                    await promise;
        ...
        it("--wait 成功终态输出详情与全文；失败终态非零退出并带出失败现场", async () => {
            ...
            const { port } = await listen((request, response) => {
                ...
                if (url.endsWith("/output")) {
                    response.end(JSON.stringify({ jobId: url, output: "fatal: not a git repo\nexit 2\n" }));
                    return;
                }
                // GET /api/jobs/:id：第一个 Job 直接 done，第二个返回 error
                response.end(JSON.stringify(phase === 1
                    ? jobInfo({
                        jobId: "job-1",
                        status: JobStatus.DONE,
                        result: { exitCode: 0 },
            ...
            const failLines = [];
            await expect(runJobsCommand("run", ["ws", "--wait", "--", "make", "check"], {
                paths,
                processEnv,
                pollIntervalMs: 1,
