...
export function defaultConfigPaths(cwd = process.cwd()) {
    return {
...
export function assertEnvironmentName(name) {
    if (!ENVIRONMENT_NAME_RE.test(name) ||
        name === "__proto__" ||
        name === "constructor" ||
        name === "prototype") {
...
export function parseCliConfig(value) {
...
export async function loadCliConfig(path, options = {}) {
...
export async function localProjectConfigTarget(cwd) {
    ...
    for (;;) {
        ...
        if (parent === directory)
            return join(normalizedCwd, PROJECT_CONFIG_FILE);
        directory = parent;
    }
}
/** 原子写入用户级配置；POSIX 下目录与文件分别限制为 0700/0600。 */
export async function saveCliConfig(path, config) {
...
export async function findProjectConfig(cwd) {
