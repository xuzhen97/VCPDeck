...
describe("frp command", () => {
    it("未知子命令与未知选项返回错误", async () => {
        ...
        await expect(runFrpCommand("instances", ["--watch"])).rejects.toThrow("未知选项");
    });
    ...
    it("mapping create 的 --env 与机器名解析使用同一环境", async () => {
        ...
        const { paths, processEnv } = await fixture(port);
        await expect(runFrpCommand("mapping", ["create", "nas", "--local-port=1919", "--env=test"], { paths, processEnv, pollIntervalMs: 1 })).rejects.toMatchObject({ code: "FRP_PROXY_CONFIRM_TIMEOUT" });
    });
    it("mapping create TCP name 可选，等待 active 后输出最终映射", async () => {
        ...
        const { paths, processEnv } = await fixture(port);
        const lines = [];
        await runFrpCommand("mapping", ["create", "nas", "--local-port=1919", "--json"], { paths, processEnv, log: (message) => lines.push(message), pollIntervalMs: 1 });
    ...
    it.each(["http", "https"])("mapping create %s 传递 domain/name/instance", async (proxyType) => {
        let createBody = {};
        const { port } = await listen((request, response) => {
            let body = "";
            request.on("data", (chunk) => (body += chunk));
    ...
    it("mappings 支持 --client 名称解析过滤", async () => {
        ...
        expect(requested.some((u) => u.startsWith("/api/frp/mappings?clientId=c9"))).toBe(true);
        const output = lines.join("\n");
    ...
    it("mapping create 遇到 busy 只提示稍后查询，且不重试 POST", async () => {
        ...
        const { paths, processEnv } = await fixture(port);
        await expect(runFrpCommand("mapping", ["create", "nas", "--local-port=1919"], {
            paths,
            processEnv,
            pollIntervalMs: 1,
    ...
    it("mapping delete 遇到 busy 只提示稍后查询，且不重试 DELETE", async () => {
        ...
        const { paths, processEnv } = await fixture(port);
        await expect(runFrpCommand("mapping", ["delete", "fm_1"], {
            paths,
            processEnv,
            pollIntervalMs: 1,
        })).rejects.toThrow("映射正在自动恢复，请稍后运行 frp mappings 查看进度");
