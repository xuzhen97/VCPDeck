...
import { loadReconnectToken, runTerminalCommand, } from "./terminal-command.js";
...
describe("terminal command", () => {
    it("未知子命令与缺参数返回错误；--help 零开销输出", async () => {
        await expect(runTerminalCommand("create", [])).rejects.toThrow("Terminal 命令");
        await expect(runTerminalCommand(undefined, ["--help"])).resolves.toBeUndefined();
        await expect(runTerminalCommand("close", ["ws"])).rejects.toThrow("Terminal 命令");
    });
    it("shells 列出可用 shell", async () => {
        const server = createServer((request, response) => {
            response.setHeader("content-type", "application/json");
            if ((request.url ?? "").split("?")[0] === "/api/clients") {
                response.end(JSON.stringify([{ clientId: "c1", name: "ws", online: true }]));
                return;
            }
    ...
    it("close 先取详情再删除，输出目标摘要", async () => {
        ...
        const server = createServer((request, response) => {
            ...
            if (path === "/api/clients/c1/terminals/t-9" &&
                request.method === "GET") {
                response.end(JSON.stringify({
                    sessionId: "t-9",
                    clientId: "c1",
        ...
        expect(requests.some((r) => r.method === "DELETE" && r.path.endsWith("/terminals/t-9"))).toBe(true);
        expect(lines.join("\n")).toContain("已关闭");
        expect(lines.join("\n")).toContain("创建者 admin");
    });
});
describe("terminal attach 重连令牌", () => {
    class FakeSocket {
        ...
        emit(event, payload, cb) {
            if (event === "terminal:attach") {
                this.attachPayloads.push(payload);
    ...
    it("attach 后持久化重连令牌；再次 attach 回传令牌恢复操作权", async () => {
        ...
        const storePath = join(dirname(paths.globalConfigPath), "terminal-reconnect.json");
        await waitFor(() => loadReconnectToken(storePath, "ts-1").then((t) => t === "rt_1"));
        ...
        await waitFor(() => loadReconnectToken(storePath, "ts-1").then((t) => t === "rt_2"));
        // 收尾：Ctrl+Q 清理，避免悬挂句柄
