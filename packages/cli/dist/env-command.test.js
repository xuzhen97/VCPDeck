...
describe("env command", () => {
    it("添加、查看、设置全局和项目默认并删除环境", async () => {
        ...
        expect(JSON.parse(await readFile(join(state.paths.cwd, "..", ".vcpdeck.json"), "utf8"))).toEqual({ version: 1, environment: "dev" });
        expect(state.logs.join("\n")).not.toContain("secret");
        await runEnvCommand("remove", ["dev"], commandContext);
        expect((await loadCliConfig(state.paths.globalConfigPath)).environments).toEqual({});
    });
    it("默认把 token-env 注册为 Bearer，并拒绝认证参数混用", async () => {
        ...
        await expect(runEnvCommand("add", args, commandContext)).rejects.toThrow("环境已存在");
        await expect(runEnvCommand("add", [
            "bad",
            "--server=https://deck.example",
            "--auth=bearer",
            "--username=admin",
            "--token-env=TOKEN",
        ], commandContext)).rejects.toThrow("不接受");
    });
