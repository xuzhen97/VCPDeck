...
export async function runTerminalCommand(subcommand, argv, context = {}) {
...
async function runShells(argv, context) {
    ...
    log(formatTable(shells.map((shell) => ({
        id: shell.id,
        label: shell.label,
        kind: shell.kind,
...
async function runNew(argv, context) {
    ...
    const shell = shells.find((item) => item.id === shellOption) ??
...
async function runList(argv, context) {
    ...
    log(formatTable(sessions.map((session) => ({
...
async function runAttach(argv, context) {
    ...
    const socketFactory = context.socketFactory ?? ((url, auth) => ioClient(url, { auth, transports: ["websocket"] }));
    ...
    socket.emit(Events.TERMINAL_ATTACH, reconnectToken ? { sessionId, reconnectToken } : { sessionId }, async (response) => {
        const ack = response;
        if (!ack?.ok) {
            cleanup();
            process.exitCode = 1;
...
/** 重连令牌本地存储路径（与全局配置同目录）。 */
export function reconnectStorePath(context) {
    const globalConfigPath = context.paths?.globalConfigPath ??
...
export async function loadReconnectToken(storePath, sessionId) {
    try {
...
/** 保存/更新会话重连令牌。 */
export async function saveReconnectToken(storePath, sessionId, token) {
    let store = {};
    try {
...
/** 会话结束后移除重连令牌。 */
export async function removeReconnectToken(storePath, sessionId) {
    try {
        const store = JSON.parse(await readFile(storePath, "utf8"));
