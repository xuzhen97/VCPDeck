...
import { findProjectConfig, localProjectConfigTarget, parseCliConfig, parseProjectConfig, saveCliConfig, } from "./config.js";
...
describe("CLI config files", () => {
    it("查找最近父目录配置并在 Git 根停止", async () => {
        ...
        await writeFile(join(root, ".vcpdeck.json"), JSON.stringify({ version: 1, environment: "dev" }));
        expect(await findProjectConfig(nested)).toBe(join(root, ".vcpdeck.json"));
        expect(await localProjectConfigTarget(nested)).toBe(join(root, ".vcpdeck.json"));
    });
