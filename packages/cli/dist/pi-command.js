...
import { findClientByClientId, fetchClientRoots, resolveClientId, } from "./client-resolver.js";
...
export async function runPiCommand(subcommand, argv, context = {}) {
...
async function runRun(argv, context) {
    ...
    await waitUntilIdle(client, clientId, sessionId, cwdRef, waitTimeout, (s) => progressLog(`[vcpdeck] Pi 状态: ${s}`), context.pollIntervalMs ?? POLL_INTERVAL_MS);
...
async function runAttachRepl(argv, context) {
    ...
    const perPromptTimeout = parsePositiveSeconds(stringOption(options, "timeout"), "--timeout") ??
