...
function release(status, clientStates = {}) {
    return {
        version: "1.2.3",
        archives: {},
        status,
        ...
        clientStates: Object.fromEntries(Object.entries(clientStates).map(([clientId, entry]) => [
            clientId,
...
describe("release command", () => {
    it("先校验本地构件，再要求环境配置", async () => {
        await expect(runReleaseCommand("upload", [
    ...
    it("Local 后端协商后使用命名 Bearer 环境上传两个平台构件", async () => {
        ...
        const { port } = await listen((request, response) => {
            ...
            request.on("end", () => {
                ...
                response.end(JSON.stringify(request.url === "/api/releases/uploads"
                    ? { mode: "server" }
        ...
        expect(requests.every((request) => request.authorization === "Bearer vcp_test_token")).toBe(true);
        const raw = requests.filter((request) => request.url.startsWith("/api/releases/upload?"));
        expect(raw.map((request) => request.body)).toEqual([
            "win-archive",
            "linux-archive",
        ]);
        expect(requests.filter((request) => request.url === "/api/releases/uploads")).toHaveLength(2);
        expect(logs.join("\n")).toContain("环境: test");
    ...
    it("Alibaba 模式只向 Provider 直传构件并在 403 时刷新 URL", async () => {
        ...
        expect(controlRequests.some((request) => request.url.startsWith("/api/releases/upload?"))).toBe(false);
        expect(providerRequests.map((request) => request.body)).toEqual([
            "win-archive",
            "win-archive",
            "linux-archive",
        ]);
        expect(controlRequests.some((request) => request.url.endsWith("/parts"))).toBe(true);
    ...
    it("拒绝非 HTTPS Provider 分片 URL，且不发送构件正文", async () => {
        ...
        await expect(runReleaseCommand("upload", [win, linux], {
            paths: state.paths,
            processEnv: state.processEnv,
            directFetch: directFetch,
            log: () => undefined,
    ...
    it("upload --wait 上传后直接验收终态", async () => {
        const { port } = await listen((request, response) => {
            ...
            if (request.method === "POST") {
                request.resume();
                request.on("end", () => response.end(JSON.stringify(request.url === "/api/releases/uploads"
                    ? { mode: "server" }
                    : { release: release("uploaded") })));
                return;
            }
            response.end(JSON.stringify(request.url?.startsWith("/api/releases?")
                ? {
    ...
    it("wait 容忍 Server 暂时不可达，并等待全部 Client 成功", async () => {
        ...
        const { port } = await listen((request, response) => {
            ...
            response.end(JSON.stringify({ serverVersion: "1.2.3", activeRelease: null }));
        });
        const state = await fixture(port);
        const logs = [];
    ...
    it("wait 在 Release failed 时立即返回失败", async () => {
        ...
        const state = await fixture(port);
        await expect(runReleaseCommand("wait", ["1.2.3", "--timeout=1"], {
            paths: state.paths,
            processEnv: state.processEnv,
            pollIntervalMs: 10,
            requestTimeoutMs: 500,
