...
describe("parseCommandArgs", () => {
    it("解析长选项、布尔选项与位置参数", () => {
        const { positionals, options } = parseCommandArgs(["run", "--env=prod", "--wait", "x"], { value: ["env"], boolean: ["wait"] });
        expect(positionals).toEqual(["run", "x"]);
        expect(options).toEqual({ env: "prod", wait: true });
    });
    it("裸 -- 分隔符后的参数原样作为位置参数，不再解析为选项", () => {
        const { positionals, options } = parseCommandArgs(["run", "--wait", "--", "git", "status", "--porcelain", "-z"], { value: [], boolean: ["wait"] });
