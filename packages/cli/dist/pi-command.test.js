 ...
 */
async function listenPi(options = {}) {
    const bodies = [];
    let stateIndex = 0;
    const server = createServer((request, response) => {
        ...
        if (path === "/api/jobs" && request.method === "POST") {
            ...
            response.end(JSON.stringify({ jobId: "root-job", status: "running", type: "file.roots" }));
            return;
        }
        ...
        if (path.endsWith("/pi/agent/new") && request.method === "POST") {
            ...
            request.on("end", () => {
                ...
                response.end(JSON.stringify({ sessionId: "s-new", jobId: "job-new" }));
            });
            return;
        }
        ...
        if (/\/pi\/agent\/(s-[a-z0-9-]+)$/.test(path) && request.method === "POST") {
            ...
            request.on("end", () => {
                ...
                response.end(JSON.stringify({ submissionId: "sub-1", accepted: true }));
            });
            return;
        }
        if (/\/pi\/agent\/(s-[a-z0-9-]+)$/.test(path) && request.method === "GET") {
            const sequence = options.stateSequence && options.stateSequence.length > 0
                ? options.stateSequence
                : ["idle"];
            const status = stateIndex < sequence.length ? sequence[stateIndex] : "idle";
            stateIndex += 1;
            response.end(JSON.stringify({
                status,
                streaming: false,
        ...
        if (path.includes("/sessions/s-") && path.includes("/context")) {
            response.end(JSON.stringify({ messages: options.contextMessages ?? [] }));
            return;
        }
...
describe("pi command", () => {
    ...
    it("models 输出 provider/modelId；多根且缺省 --root 时 fail closed", async () => {
        ...
        await expect(runPiCommand("models", ["workstation"], {
            paths: second.paths,
    ...
    it("run 自动创建会话、提交提示词、轮询至 idle 并提取助手回复", async () => {
        ...
        const lines = [];
        await runPiCommand("run", ["workstation", "列出", "当前", "文件", "--cwd=proj", "--json"], {
            paths,
            processEnv,
            pollIntervalMs: 1,
            log: (m) => lines.push(m),
        });
        const newCall = bodies.find((b) => b.kind === "new");
        expect(newCall.body.rootDir).toBe("D:\\");
        const promptCall = bodies.find((b) => b.kind === "prompt");
    ...
    it("既有会话走 open；扩展输入等待时明确报错", async () => {
        ...
        await expect(runPiCommand("run", ["workstation", "继续", "--session=s-existing", "--root=D:\\"], { paths: first.paths, processEnv: first.processEnv, pollIntervalMs: 1, log: () => { } })).rejects.toThrow("等待扩展输入");
        const idle = await listenPi({ contextMessages: [
                { role: "assistant", content: [{ type: "text", text: "完成" }] },
            ] });
        const second = await fixture(idle.port);
        const lines = [];
        await runPiCommand("run", ["workstation", "收尾", "--session=s-exist", "--root=D:\\"], { paths: second.paths, processEnv: second.processEnv, pollIntervalMs: 1, log: (m) => lines.push(m) });
    ...
    describe("pi attach REPL", () => {
        ...
        it("多轮对话：每轮提交提示词、取回回复；/exit 退出", async () => {
            ...
            const prompts = bodies.filter((b) => b.kind === "prompt");
