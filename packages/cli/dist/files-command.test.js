...
async function listenWithJobs(options = {}) {
    const jobs = [];
    let jobSeq = 0;
...
describe("files command", () => {
    ...
    it("--root 显式指定时不探测；多根且缺省时报错要求指定", async () => {
        ...
        await expect(runFilesCommand("list", ["workstation", "x"], {
            paths: second.paths,
    ...
    it("read 输出文本内容；失败 Job 转为带错误码的异常", async () => {
        const ok = await listenWithJobs({
            onJob: ({ type }) => type === "file.readText" ? { content: "hello\nworld\n", size: 13 } : {},
        });
        const first = await fixture(ok.port);
        const lines = [];
        await runFilesCommand("read", ["workstation", "notes.txt", "--max-bytes=1024"], {
            paths: first.paths,
            processEnv: first.processEnv,
            pollIntervalMs: 1,
            log: (message) => lines.push(message),
        ...
        await expect(runFilesCommand("read", ["workstation", "missing.txt"], {
            paths: second.paths,
            processEnv: second.processEnv,
            pollIntervalMs: 1,
            log: () => { },
    ...
    describe("files 写操作", () => {
        it("write 用 --input 本地文件作为内容并覆盖写入", async () => {
            const inputPath = join(await mkdtemp(join(tmpdir(), "vcpdeck-files-write-")), "content.txt");
            tempDirectories.push(inputPath); // rm 递归删除父目录即可
            await writeFile(inputPath, "hello vcpdeck\n", "utf8");
            const { port, jobs } = await listenWithJobs({
                roots: ["D:\\"],
                onJob: ({ type }) => type === "file.writeText" ? { path: "D:/x.txt" } : {},
            });
            const { paths, processEnv } = await fixture(port);
            const lines = [];
            await runFilesCommand("write", ["workstation", "x.txt", "--input=" + inputPath, "--json"], { paths, processEnv, pollIntervalMs: 1, log: (m) => lines.push(m) });
        ...
        it("mkdir/move 的 payload 形状与覆盖语义正确", async () => {
            ...
            await runFilesCommand("move", ["workstation", "a.txt", "b.txt", "--overwrite"], { paths, processEnv, pollIntervalMs: 1, log: () => { } });
        ...
        it("失败 Job 转为带稳定错误码的异常（如目录非空 PATH_CONFLICT）", async () => {
            const { port } = await listenWithJobs({
                roots: ["D:\\"],
                onJob: ({ type }) => type === "file.delete" ? { fail: "PATH_CONFLICT" } : { path: "ok" },
            });
            const { paths, processEnv } = await fixture(port);
            await expect(runFilesCommand("delete", ["workstation", "docs"], {
                paths,
                processEnv,
                pollIntervalMs: 1,
                log: () => { },
    ...
    describe("文件传输", () => {
        it("download 经导出 Job 与签名 URL 拉取并校验 sha256", async () => {
            ...
            let downloadTokenRequested;
            const server = createServer((request, response) => {
                const url = request.url ?? "";
                response.setHeader("content-type", "application/json");
                if (url === "/api/clients") {
                    response.end(JSON.stringify([{ clientId: "c1", name: "workstation", online: true }]));
                    return;
                }
                if (url === "/api/jobs" && request.method === "POST") {
                    const chunks = [];
            ...
            const { paths, processEnv } = await fixture(port);
            const localPath = join(await mkdtemp(join(tmpdir(), "vcpdeck-files-dl-")), "out.bin");
            const lines = [];
            await runFilesCommand("download", ["workstation", "logs/app.log", localPath, "--root=D:\\"], { paths, processEnv, pollIntervalMs: 1, log: (m) => lines.push(m) });
        ...
        it("download sha256 不一致时删除本地半成品并报错", async () => {
            ...
            const { paths, processEnv } = await fixture(port);
            const localPath = join(await mkdtemp(join(tmpdir(), "vcpdeck-files-dl-")), "out.bin");
            await expect(runFilesCommand("download", ["workstation", "a.log", localPath, "--root=D:\\"], { paths, processEnv, pollIntervalMs: 1, log: () => { } })).rejects.toThrow("SHA-256 不一致");
        ...
        it("upload 直传分片到预签名 URL 并等待导入终态", async () => {
            ...
            const server = createServer((request, response) => {
                ...
                if (url === "/api/files/upload-sessions/up-1/complete" &&
                    request.method === "POST") {
                    response.end(JSON.stringify({
                        jobId: "imp-1",
                        status: "running",
            ...
            const lines = [];
            await runFilesCommand("upload", ["workstation", localPath, "x.txt", "--root=D:\\"], { paths, processEnv, pollIntervalMs: 1, log: (m) => lines.push(m) });
