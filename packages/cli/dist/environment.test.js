...
describe("resolveEnvironment", () => {
    it("按 flag > 环境变量 > 项目 > 全局默认的顺序选择", async () => {
        ...
        await writeFile(join(project, "..", "..", ".vcpdeck.json"), JSON.stringify({ version: 1, environment: "project" }));
        const processEnv = {
        ...
        expect(await resolveEnvironment({
            environment: "flag",
            paths,
            processEnv,
    ...
    it("项目配置损坏或引用未知环境时 fail closed", async () => {
        ...
        await expect(resolveEnvironment({
            paths,
    ...
    it("凭据变量缺失时失败，current 模式可只解析安全摘要", async () => {
        ...
        await expect(resolveEnvironment({ environment: "flag", paths, processEnv: {} })).rejects.toThrow("FLAG_TOKEN");
