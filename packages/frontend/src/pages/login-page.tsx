		...
		<div className="vcpdeck-background grid min-h-dvh lg:grid-cols-[1.1fr_.9fr]">
			<section className="hidden items-end p-12 lg:flex">
				<div className="max-w-xl">
					<div className="mb-6 flex items-center gap-3 text-primary"><Boxes className="size-9" /><span className="text-xl font-semibold">VCPDeck</span></div>
					<h1 className="text-5xl font-semibold leading-tight">连接机器，执行任务，保持全局清晰。</h1>
					<p className="mt-5 text-lg text-muted-foreground">个人 AI 协作驾驶台，为远程操作提供单一、可信的工作上下文。</p>
				</div>
			</section>
			<section className="flex items-center justify-center p-5">
				<Button className="absolute right-5 top-5" size="icon" variant="ghost" onClick={toggleTheme} aria-label="切换主题">{theme === "dark" ? <Sun /> : <Moon />}</Button>
				<Card className="w-full max-w-md">
					<CardHeader><CardTitle>登录 VCPDeck</CardTitle><CardDescription>使用你的 Web 身份继续</CardDescription></CardHeader>
					<CardContent>
