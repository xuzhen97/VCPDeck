...
export function TokensPanel() {
	...
	const load = useCallback(
		(signal: AbortSignal) => sdk.auth.tokens.list(signal),
		[sdk],
	);
	...
	if (resource.error)
		return <ErrorState message="无法加载 Token" onRetry={resource.reload} />;
	return (
		<div className="space-y-5">
			<Card>
				<CardHeader>
					<CardTitle>创建 CLI Token</CardTitle>
				</CardHeader>
				<CardContent>
					<form className="flex max-w-xl gap-2" onSubmit={create}>
			...
			>
				<DialogContent>
					<DialogTitle>立即保存 Token</DialogTitle>
					<DialogDescription>
						此 Token 只显示一次，关闭后无法再次查看。
					</DialogDescription>
					<pre className="mt-4 overflow-x-auto rounded-lg bg-black/30 p-3 text-sm">
						{created?.token}
					</pre>
					<div className="mt-5 flex justify-end">
						<Button onClick={() => setCreated(undefined)}>我已保存</Button>
					</div>
				</DialogContent>
			</Dialog>
			<Dialog
