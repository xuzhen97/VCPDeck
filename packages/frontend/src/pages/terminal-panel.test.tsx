...
function makeSdk(overrides: Record<string, unknown> = {}) {
	...
	const shells = vi.fn(async () => [
		{ id: "bash", label: "bash", kind: "bash", isDefault: true },
	]);
	const create = vi.fn(
		async (
			_clientId: string,
			body: { shellId: string; cols: number; rows: number },
...
describe("TerminalPanel", () => {
	...
	it("新建终端：菜单选择 Shell 并调用 create（默认尺寸）", async () => {
		...
		await user.click(
			within(await screen.findByTestId("terminal-new-menu")).getByText("bash"),
		);
		await waitFor(() =>
		...
		const { sdk } = makeSdk({
			list: vi.fn(async () => ({
				data: Array.from({ length: 5 }, (_, i) =>
					sessionInfo({ sessionId: `s${i}`, status: "active" }),
				),
				total: 5,
				page: 1,
				pageSize: 50,
	...
	it("已结束会话的标签可直接清除（不弹确认、不调 remove）", async () => {
		const user = userEvent.setup();
		const { sdk, remove } = makeSdk({
			list: vi.fn(async () => ({
				data: [
					...
					sessionInfo({ sessionId: "s2", status: "detached" }),
				],
				total: 2,
				page: 1,
				pageSize: 50,
		...
		// 终态标签显示清除按钮，点击后标签消失，剩余标签重新编号
		await user.click(screen.getByRole("button", { name: "清除终端 bash 1" }));
		await waitFor(() => expect(screen.queryByText(/bash 2/)).toBeNull());
		expect(screen.getByText(/bash 1/)).toBeTruthy();
		expect(remove).not.toHaveBeenCalled();
	});

	it("已清除的终态标签在新建/刷新后不复活", async () => {
		const user = userEvent.setup();
		let listData = [
			sessionInfo({
				sessionId: "s9",
			...
			sessionInfo({ sessionId: "s8", status: "detached" }),
		];
		const { sdk, create, remove } = makeSdk({
			list: vi.fn(async () => ({
				data: listData,
		...
		await screen.findByTestId("terminal-tabs");
		await user.click(screen.getByRole("button", { name: "清除终端 bash 1" }));
		await waitFor(() => expect(screen.queryByText(/closed/)).toBeNull());
		// 新建 shell 触发 reload：已清除的 closed 标签不再出现
		listData = [
			sessionInfo({
				sessionId: "s9",
				status: "closed",
			...
			sessionInfo({ sessionId: "s8", status: "detached" }),
			sessionInfo({ sessionId: "s10", status: "detached" }),
		];
		await user.click(screen.getByRole("button", { name: /新建/ }));
		await user.click(
			within(await screen.findByTestId("terminal-new-menu")).getByText("bash"),
	...
	it("关闭终端需二次确认并调用 remove", async () => {
		...
		renderPanel(sdk);
		await screen.findByTestId("terminal-tabs");
		// 关闭入口在 tab 的 X（控制条已移除“关闭终端”按钮）
		await user.click(screen.getByRole("button", { name: "关闭终端 bash 1" }));
	...
	it("放大终端到整页，再次点击还原", async () => {
		...
		await user.click(screen.getByRole("button", { name: "放大终端" }));
		expect(panel().className).toContain("fixed inset-0");
		await user.click(screen.getByRole("button", { name: "还原终端" }));
		expect(panel().className).not.toContain("fixed inset-0");
	});
	...
	it("列表加载失败展示稳定错误并支持重试", async () => {
		...
		await waitFor(() =>
			expect(
				screen.getByText(/终端不可用：TERMINAL_CLIENT_OFFLINE/),
			).toBeTruthy(),
		);
