...
const identity = (isAdmin: boolean): IdentityInfo => ({ id: "i1", username: "operator", displayName: "操作员", isAdmin, disabledAt: null, createdAt: "2026-07-26T00:00:00.000Z" });

function LocationProbe() { return <output aria-label="当前位置">{useLocation().pathname}</output>; }

function renderSettings(isAdmin = true, path = "/settings/tokens") {
	const token = { id: "t1", token: "vcp_secret_once", label: "CLI" };
	const tokens = { list: vi.fn().mockResolvedValue([]), create: vi.fn().mockResolvedValue(token), revoke: vi.fn().mockResolvedValue({ ok: true }) };
	const client = { auth: { me: async () => identity(isAdmin), updateMe: vi.fn(), tokens }, identities: { list: vi.fn().mockResolvedValue([]), create: vi.fn(), disable: vi.fn(), enable: vi.fn() } } as unknown as VcpDeckClient;
	render(<MemoryRouter initialEntries={[path]}><SdkProvider client={client}><AuthProvider><AppRoutes /><LocationProbe /></AuthProvider></SdkProvider></MemoryRouter>);
	return { tokens };
}
...
describe("SettingsPage", () => {
	...
	it("requires confirmation before revoking a token", async () => {
		const { tokens } = renderSettings();
		tokens.list.mockResolvedValue([{ id: "t1", label: "CLI", lastUsedAt: null, expiresAt: null, revokedAt: null, createdAt: "2026-07-26T00:00:00.000Z" }]);
	...
	it("hides and blocks identity management for non-admin users", async () => {
		...
		await waitFor(() => expect(screen.getByLabelText("当前位置")).toHaveTextContent("/settings/profile"));
