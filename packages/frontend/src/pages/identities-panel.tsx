...
export function IdentitiesPanel() {
	...
	const load = useCallback(
		(signal: AbortSignal) => sdk.identities.list(signal),
		[sdk],
	);
	...
	async function disable() {
		if (!disabling) return;
		await sdk.identities.disable(disabling.id);
		setDisabling(undefined);
		resource.reload();
	}
	async function enable(identity: IdentityInfo) {
		await sdk.identities.enable(identity.id);
		resource.reload();
	}

	if (resource.loading) return <LoadingState label="正在加载身份…" />;
	if (resource.error)
		return <ErrorState message="无法加载身份" onRetry={resource.reload} />;
	return (
		<div className="space-y-5">
			<Card>
				<CardHeader>
					<CardTitle>创建身份</CardTitle>
				</CardHeader>
				<CardContent>
					<form
					...
					>
						...
						<div>
							<Label htmlFor="identity-password">初始密码</Label>
							<Input
			...
			<Card>
				<CardHeader>
					<CardTitle>身份列表</CardTitle>
				</CardHeader>
				<CardContent>
					{resource.data?.map((identity) => (
						<article
							key={identity.id}
						...
						>
							<div>
								...
								<p className="text-xs text-muted-foreground">
									{identity.disabledAt ? "已禁用" : "已启用"}
								</p>
							</div>
							{identity.disabledAt ? (
								<Button variant="outline" onClick={() => enable(identity)}>
									启用
								</Button>
							) : (
								<Button
									variant="destructive"
									onClick={() => setDisabling(identity)}
								>
									禁用
								</Button>
							)}
			...
			>
				<DialogContent>
					<DialogTitle>禁用身份？</DialogTitle>
					<DialogDescription>
						该身份的会话和 Token 将无法继续访问。
					</DialogDescription>
					<div className="mt-5 flex justify-end gap-2">
						<Button variant="outline" onClick={() => setDisabling(undefined)}>
							取消
						</Button>
