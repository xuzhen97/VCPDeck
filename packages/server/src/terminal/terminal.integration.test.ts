...
function makeHarness() {
	...
	const gateway = new ClientGateway(
		...
		{ markInactiveByClientId: vi.fn(async () => {}), updateStatus: vi.fn() } as never,
		{ bindEmitter: vi.fn(), request: vi.fn(), resolve: vi.fn(), disconnect: vi.fn() } as never,
		{ publish: vi.fn(), stream: vi.fn() } as never,
		{
			markReconcilePending: vi.fn(async () => {}),
			reconcileGeneration: vi.fn(async () => ({ acceptedRunIds: [], closedRunIds: [], reportAgain: false })),
			withReconciledSocket: vi.fn(async (_c: string, _s: string, op: () => Promise<void>) => op()),
	...
	broker.bindEmitter((socketId, request) => {
		void fakeClient.broker.request({ clientId: "c1", socketId }, request).then((response) => {
			void gateway.handleTerminalResponse({ id: socketId, data: { clientId: "c1" } } as never, response as never);
		});
	});
...
async function registerClient(h: ReturnType<typeof makeHarness>) {
	...
	h.memory.clients.set("c1", { id: "c1", socketId: "client-sock-1", online: true });
	h.fakeClient.bindClientSocket("client-sock-1");
	// Client 注册后上报状态对账
	await h.service.handleClientState("c1", "client-sock-1", { clientId: "c1", generationId: "g1", sessions: [] });
}
...
describe("终端端到端回路", () => {
	it("创建 → attach → 输入 → PTY 回显 → 浏览器收到 snapshot + output", async () => {
		...
		const attached = await h.service.attachBrowser({ sessionId: created.sessionId, actor: ACTOR, socketId: "browser-1" });
		expect(attached.mode).toBe("operator");
		await h.service.whenAttachSettled(created.sessionId);
		const snap = h.browserEmits.find((e) => e.event === Events.TERMINAL_SNAPSHOT);
		expect(snap?.payload).toMatchObject({ sessionId: created.sessionId, snapshot: `SNAP:${created.sessionId}` });

		await h.service.browserInput({ socketId: "browser-1", sessionId: created.sessionId, attachmentId: attached.attachmentId, data: "ls\r" });
		expect(h.fakeClient.receivedInput).toEqual([{ sessionId: created.sessionId, data: "ls\r" }]);
		await new Promise((resolve) => setTimeout(resolve, 10));
		const outputs = h.browserEmits.filter((e) => e.event === Events.TERMINAL_OUTPUT);
		expect(outputs.map((o) => (o.payload as { data: string }).data)).toEqual(["echo:ls\r"]);
	...
	it("第二浏览器只读：伪造 input 被拒绝且不达 Client", async () => {
		...
		await h.service.attachBrowser({ sessionId: created.sessionId, actor: ACTOR, socketId: "b1" });
		const viewer = await h.service.attachBrowser({ sessionId: created.sessionId, actor: { ...ACTOR, identityId: "id2" }, socketId: "b2" });
		await expect(
			h.service.browserInput({ socketId: "b2", sessionId: created.sessionId, attachmentId: viewer.attachmentId, data: "rm -rf /" }),
	...
	it("operator 断开后 token 重绑恢复操作权", async () => {
		...
		await registerClient(h);
		const created = await h.service.createSession("c1", { shellId: "bash", cols: 80, rows: 24 }, ACTOR);
		const first = await h.service.attachBrowser({ sessionId: created.sessionId, actor: ACTOR, socketId: "b1" });
		...
		await h.service.browserInput({ socketId: "b2", sessionId: created.sessionId, attachmentId: rebind.attachmentId, data: "pwd\r" });
	...
	it("保护期后 viewer 接管成功", async () => {
		...
		const viewer = await h.service.attachBrowser({ sessionId: created.sessionId, actor: { ...ACTOR, identityId: "id2" }, socketId: "b2" });
		await expect(
			h.service.browserTakeover({ socketId: "b2", sessionId: created.sessionId, attachmentId: viewer.attachmentId }),
		...
		const winner = await h.service.browserTakeover({ socketId: "b2", sessionId: created.sessionId, attachmentId: viewer.attachmentId });
	...
	it("Client 重启对账：旧会话 interrupted，孤儿 closeSessionIds", async () => {
		...
		expect(h.memory.sessions.get(created.sessionId)?.status).toBe("interrupted");
		expect(h.memory.sessions.get(created.sessionId)?.endReason).toBe("TERMINAL_CLIENT_RESTARTED");
		const ack2 = await h.service.handleClientState("c1", "client-sock-2", {
			clientId: "c1",
			generationId: "g2",
			sessions: [{ sessionId: "ts_orphan", shellId: "bash", status: "active", cols: 80, rows: 24, lastSeq: 0 }],
		});
	...
	it("安全 canary：输入/路径/token 不进入 DB 与审计", async () => {
		...
		await registerClient(h);
		const created = await h.service.createSession("c1", { shellId: "bash", cols: 80, rows: 24 }, ACTOR);
		const attached = await h.service.attachBrowser({ sessionId: created.sessionId, actor: ACTOR, socketId: "b1" });
		...
		expect(JSON.stringify(h.memory.audits)).not.toContain(attached.reconnectToken);
	});
});
