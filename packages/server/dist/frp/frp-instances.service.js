...
var FrpsInstancesService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.FrpsInstancesService = exports.FrpsDashboardError = void 0;
const common_1 = require("@nestjs/common");
const node_crypto_1 = require("node:crypto");
const net = __importStar(require("node:net"));
const prisma_service_js_1 = require("../prisma/prisma.service.js");
/** FRPS Dashboard 写操作确认失败。 */
class FrpsDashboardError extends Error {
    code;
...
exports.FrpsDashboardError = FrpsDashboardError;
let FrpsInstancesService = FrpsInstancesService_1 = class FrpsInstancesService {
    prisma;
    logger = new common_1.Logger(FrpsInstancesService_1.name);
    constructor(prisma) {
        this.prisma = prisma;
    }
    async create(dto) {
        ...
        const id = `frps_${(0, node_crypto_1.randomUUID)().slice(0, 8)}`;
    ...
    async update(id, dto) {
        ...
        if (!existing)
            throw new Error(`FrpsInstance "${id}" 不存在`);
        ...
        if (dto.name !== undefined)
            data.name = dto.name;
        if (dto.serverAddr !== undefined)
            data.serverAddr = dto.serverAddr;
        if (dto.serverPort !== undefined)
            data.serverPort = dto.serverPort;
        if (dto.authToken !== undefined)
            data.authToken = dto.authToken;
        if (dto.dashboardScheme !== undefined)
            data.dashboardScheme = dto.dashboardScheme;
        if (dto.dashboardHost !== undefined)
            data.dashboardHost = dto.dashboardHost;
        if (dto.dashboardPort !== undefined)
            data.dashboardPort = dto.dashboardPort;
        if (dto.dashboardUser !== undefined)
            data.dashboardUser = dto.dashboardUser;
        ...
        if (dto.portRangeEnd !== undefined)
            data.portRangeEnd = dto.portRangeEnd;
    ...
    async delete(id) {
        ...
        if (mappingCount > 0) {
            throw new Error(`无法删除：${mappingCount} 个映射关联到实例 "${existing.name}"`);
        }
    ...
    async getDefault() {
        ...
        if (!row)
            throw new Error("没有默认 FRP 实例，请先配置");
        return this.toApi(row);
    }
    /** 设置默认实例 */
    async setDefault(id) {
    ...
    async migrateFromEnvIfNeeded() {
        const count = await this.prisma.frpsInstance.count();
        if (count > 0)
            return null;
        const row = await this.prisma.frpsInstance.create({
            data: {
                id: `frps_${(0, node_crypto_1.randomUUID)().slice(0, 8)}`,
                name: "默认（从环境变量迁移）",
                ...
                dashboardScheme: process.env.FRP_DASHBOARD_SCHEME || "http",
                dashboardHost: process.env.FRP_DASHBOARD_HOST || "127.0.0.1",
                dashboardPort: parseInt(process.env.FRP_DASHBOARD_PORT || "17500", 10),
                dashboardUser: process.env.FRP_DASHBOARD_USER || "admin",
                dashboardPassword: process.env.FRP_DASHBOARD_PASSWORD || "admin",
                portRangeStart: parseInt(process.env.FRP_PORT_RANGE_START || "20000", 10),
                portRangeEnd: parseInt(process.env.FRP_PORT_RANGE_END || "21000", 10),
                isDefault: true,
            },
        });
    ...
    async probe(id) {
        ...
        if (!instance)
            throw new Error(`FrpsInstance "${id}" 不存在`);
        // 1. TCP 连接检查
        const tcpResult = await this.probeTcp(instance.serverAddr, instance.serverPort);
        // 2. Dashboard 检查
    ...
    /** TCP 连接检查 */
    probeTcp(host, port, timeoutMs = 5000) {
    ...
    /** Dashboard 认证检查 */
    async probeDashboard(instance) {
        try {
            const auth = Buffer.from(`${instance.dashboardUser}:${instance.dashboardPassword}`).toString("base64");
            const res = await fetch(`${instance.dashboardScheme}://${instance.dashboardHost}:${instance.dashboardPort}/api/serverinfo`, {
    ...
    async listDashboardProxies(instance) {
        if (!instance.dashboardHost) {
            throw new FrpsDashboardError("FRPS_DASHBOARD_REQUIRED", "FRPS 实例必须配置 Dashboard");
        }
        const auth = Buffer.from(`${instance.dashboardUser}:${instance.dashboardPassword}`).toString("base64");
        const base = `${instance.dashboardScheme}://${instance.dashboardHost}:${instance.dashboardPort}`;
        const types = ["tcp", "http", "https"];
        try {
            const results = await Promise.all(types.map(async (proxyType) => {
                const response = await fetch(`${base}/api/proxy/${proxyType}`, {
                    headers: { Authorization: `Basic ${auth}` },
                    signal: AbortSignal.timeout(5000),
                });
                if (response.status === 401 || response.status === 403) {
                    throw new FrpsDashboardError("FRPS_DASHBOARD_AUTH_FAILED", "FRPS Dashboard 认证失败");
                }
                if (!response.ok) {
                    throw new FrpsDashboardError("FRPS_DASHBOARD_UNREACHABLE", "FRPS Dashboard 不可达");
                }
                ...
                return (body.proxies ?? [])
                    ...
                    status: (proxy.status === "online" ? "online" : "offline"),
                    remotePort: proxy.remotePort ?? proxy.conf?.remotePort ?? null,
                }));
            }));
            ...
            if (error instanceof FrpsDashboardError)
                throw error;
            throw new FrpsDashboardError("FRPS_DASHBOARD_UNREACHABLE", "FRPS Dashboard 不可达");
        }
    }
    /** 健康摘要沿用宽松语义；写操作使用 listDashboardProxies。 */
    async fetchProxyList(instance) {
        try {
...
exports.FrpsInstancesService = FrpsInstancesService;
exports.FrpsInstancesService = FrpsInstancesService = FrpsInstancesService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(prisma_service_js_1.PrismaService)),
    __metadata("design:paramtypes", [prisma_service_js_1.PrismaService])
], FrpsInstancesService);
