...
const providers_registry_js_1 = require("./providers/providers.registry.js");
const alibaba_storage_provider_js_1 = require("./providers/alibaba-storage.provider.js");
...
let StorageService = StorageService_1 = class StorageService {
    ...
    async loadProvider() {
        ...
        if (!config.signSecret) {
            config.signSecret = (0, node_crypto_1.randomUUID)();
        ...
        const ProviderClass = providers_registry_js_1.STORAGE_PROVIDERS[kind];
        if (!ProviderClass) {
            this.logger.warn(`Unknown storage kind "${kind}", falling back to local`);
            const Fallback = providers_registry_js_1.STORAGE_PROVIDERS["local"];
            this.provider = new Fallback(config);
            return;
        }
        this.provider = new ProviderClass(config);
        // 阿里云 token 刷新后写回 DB，避免服务重启后使用过期凭证
        if (this.provider instanceof alibaba_storage_provider_js_1.AlibabaStorageProvider) {
    ...
    requireDirectProvider() {
        const p = this.getProvider();
        if (typeof p.createDirectUpload !== "function") {
    ...
    async createReleaseDirectUpload(size, name) {
        return this.requireDirectProvider().createDirectUpload(size, name);
    }
    /** 刷新 Release 直传会话的指定分片 URL。 */
    async refreshReleaseDirectUploadParts(fileId, uploadId, partNumbers) {
        return this.requireDirectProvider().refreshPartUrls(fileId, uploadId, partNumbers);
    }
    /** 完成 Release 直传并由 Provider 合并分片。 */
    async completeReleaseDirectUpload(fileId, uploadId) {
        await this.requireDirectProvider().completeDirectUpload(fileId, uploadId);
    }
    /** 返回当前激活 Provider 的稳定标识。 */
    currentKind() {
        return this.getProvider() instanceof alibaba_storage_provider_js_1.AlibabaStorageProvider ? "alibaba" : "local";
    }
    ...
    async getDirectDownloadUrl(key) {
        const p = this.getProvider();
        if (typeof p.getDirectDownloadUrl !== "function")
            return null;
    ...
    async uploadStream(stream, meta) {
        return this.getProvider().upload(stream, meta);
    }
    /** 签发上传令牌，返回 FileRef */
    async createUploadToken(meta, ttlSeconds = 3600) {
        const p = this.getProvider();
        const key = `${(0, node_crypto_1.randomUUID)()}/${meta.filename.replace(/[\\/:*?"<>|]/g, "_")}`;
        const queryString = p.signUploadUrl(key, ttlSeconds);
        const expiresAt = parseInt(new URLSearchParams(queryString).get("expires") || "0", 10);
    ...
    async createDownloadToken(key, ttlSeconds = 3600) {
        const p = this.getProvider();
        if (typeof p.getExternalDownloadUrl === "function") {
            // 直连后端：阿里云下载 URL 临时有效，每次实时生成，不签名
            return p.getExternalDownloadUrl(key);
        }
    ...
    async createDirectUploadSession(size, name, dbFileId) {
    ...
    async completeDirectUploadSession(dbFileId, uploadedBytes) {
        const p = this.requireDirectProvider();
        ...
        if (file.size !== uploadedBytes) {
            throw Object.assign(new Error(`上传字节数不匹配：期望 ${file.size}，实际 ${uploadedBytes}`), { code: "SIZE_MISMATCH", statusCode: 400 });
        }
    ...
    async createExportSession(jobId, size) {
    ...
    async completeExportUpload(jobId, uploadedBytes) {
        const file = await this.prisma.file.findFirst({ where: { jobId } });
        if (!file) {
            throw Object.assign(new Error("导出任务缺少 File 记录"), {
                code: "FILE_NOT_READY",
    ...
    /** 续期直传会话指定分片的上传 URL */
    async refreshDirectPartUrls(jobId, partNumbers) {
        const file = await this.prisma.file.findFirst({ where: { jobId } });
    ...
    async updateUploadProgress(jobId, loaded) {
        const job = await this.prisma.job.findUnique({ where: { id: jobId } });
        if (!job)
            return;
        let payload = {};
    ...
    /** 接收文件流并存储 */
    async receiveUpload(key, stream, expiresAt, sig) {
        const p = this.getProvider();
        ...
        const uploadAndPersist = async (meta) => {
                    ...
                    if (reportProgress) {
                        ...
                        if (now - lastEmitAt >= 500 ||
                            ...
                            progressWrite = progressWrite
                                .then(() => this.updateJobProgress(jobId, loaded, meta.size))
                                .catch(() => { });
                        }
                    }
            ...
            try {
                ...
                if (reportProgress)
                    await this.markUploadJobError(jobId);
                throw error;
            }
        };
    ...
    async updateJobProgress(jobId, loaded, total) {
        try {
    ...
    async markUploadJobError(jobId) {
        try {
            await this.prisma.job.update({
                where: { id: jobId },
                data: {
    ...
    /** 验证下载签名并返回文件流 + 元数据 */
    async downloadVerified(key, expiresAt, sig) {
        const p = this.getProvider();
