...
exports.AlibabaOpenApiClient = exports.AlibabaOpenApiError = void 0;
class AlibabaOpenApiError extends Error {
    status;
...
exports.AlibabaOpenApiError = AlibabaOpenApiError;
class AlibabaOpenApiClient {
    ...
    async post(path, payload) {
        ...
        if (!response.ok) {
            ...
            throw new AlibabaOpenApiError(response.status, `Aliyun OpenAPI failed: HTTP ${response.status} ${text.slice(0, 200)}`);
        }
        return (await response.json());
    }
    /** 获取默认 drive 信息 */
    async getDriveInfo() {
        const data = await this.post("/adrive/v1.0/user/getDriveInfo", {});
        const driveId = String(data.default_drive_id ??
            data.defaultDriveId ??
            ...
            "");
        if (!driveId)
            throw new Error("Aliyun Drive 未返回 drive_id");
    ...
    async listChildren(input) {
        ...
        const data = await this.post("/adrive/v1.0/openFile/list", payload);
        return (data.items ?? []);
    }
    /** 创建目录 */
    async createFolder(input) {
        return this.post("/adrive/v1.0/openFile/create", {
            drive_id: input.driveId,
            parent_file_id: input.parentFileId,
            name: input.name,
            type: "folder",
    ...
    async getUploadUrl(input) {
        return this.post("/adrive/v1.0/openFile/getUploadUrl", {
            drive_id: input.driveId,
    ...
    async completeUpload(input) {
        return this.post("/adrive/v1.0/openFile/complete", {
            drive_id: input.driveId,
            file_id: input.fileId,
            upload_id: input.uploadId,
        });
    ...
    async getDownloadUrl(input) {
        return this.post("/adrive/v1.0/openFile/getDownloadUrl", {
            drive_id: input.driveId,
            file_id: input.fileId,
        });
    }
    ...
    async deleteFile(input) {
        return this.post("/adrive/v1.0/openFile/delete", {
            drive_id: input.driveId,
            file_id: input.fileId,
        });
    }
}
exports.AlibabaOpenApiClient = AlibabaOpenApiClient;
