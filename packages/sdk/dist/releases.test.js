...
describe("releases", () => {
    it("创建、刷新并完成 Release 直传会话", async () => {
        ...
        const api = createReleasesApi({ request, requestRaw: vi.fn() });
        const input = {
            version: "1.2.3",
            platform: "linux-x64",
            sha256: "a".repeat(64),
    ...
    it("通过原始 body 上传按平台归档并返回 Release", async () => {
        ...
        expect(requestRaw).toHaveBeenCalledWith("POST", `/api/releases/upload?version=1.2.3&platform=linux-x64&sha256=${"a".repeat(64)}`, {
            body: "zip-bytes",
