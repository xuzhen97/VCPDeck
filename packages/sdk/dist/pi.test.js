...
function makeClient() {
    const fetcher = vi.fn(async (_input, _init) => Response.json({ ok: true }));
...
describe("createPiApi", () => {
    it("open/complete 使用 Session Job endpoint 与 body", async () => {
        ...
        const pi = createPiApi({ request: request });
        const cwdRef = { rootDir: "D:\\", relativePath: "repo" };
        await pi.agent.open("c1", "s1", cwdRef);
        expect(request).toHaveBeenLastCalledWith("POST", "/api/clients/c1/pi/agent/s1/open", cwdRef, undefined);
        await pi.agent.complete("c1", "s1", "run-1");
        expect(request).toHaveBeenLastCalledWith("POST", "/api/clients/c1/pi/agent/s1/complete", { runId: "run-1" }, undefined);
        await pi.agent.complete("c1", "s1");
        expect(request).toHaveBeenLastCalledWith("POST", "/api/clients/c1/pi/agent/s1/complete", {}, undefined);
    });
    it("run-scoped control body 使用 runId", async () => {
        const request = vi.fn(async () => ({}));
        const pi = createPiApi({ request: request });
        await pi.agent.abort("c1", "s1", "run-1");
        expect(request).toHaveBeenLastCalledWith("POST", "/api/clients/c1/pi/agent/s1/abort", { runId: "run-1" });
    });
    it("state 严格解析响应并拒绝畸形状态", async () => {
        ...
        const pi = createPiApi({ request: request });
        await expect(pi.agent.state("c1", "s1", { rootDir: "D:\\", relativePath: "repo" })).resolves.toEqual(valid);
        request.mockResolvedValueOnce({
            ...valid,
            streaming: "yes",
        });
        await expect(pi.agent.state("c1", "s1", { rootDir: "D:\\", relativePath: "repo" })).rejects.toMatchObject({ code: "PI_PROTOCOL_INVALID" });
    });
});
describe("VcpDeckClient.pi", () => {
    it("sessions.list 编码 clientId 与查询参数", async () => {
        ...
        expect(fetcher).toHaveBeenCalledWith(expect.stringContaining("/api/clients/c%2F1/pi/sessions?"), expect.any(Object));
    ...
    it("agent.eventsPath 是 session 级且编码 clientId", () => {
        ...
        expect(client.pi.agent.eventsPath("c/1", "s/1")).toBe("/api/clients/c%2F1/pi/agent/s%2F1/events");
    });
    it("agent.prompt 发送 submissionId 与 prompt", async () => {
        ...
        expect(fetcher).toHaveBeenCalledWith(expect.stringContaining("/api/clients/c1/pi/agent/s1"), expect.objectContaining({
            method: "POST",
