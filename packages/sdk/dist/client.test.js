...
describe("VcpDeckClient", () => {
    ...
    it("uses an explicit Node.js cookie", async () => {
        ...
        expect(fetcher).toHaveBeenCalledWith("https://deck.example/api/releases/upload", expect.objectContaining({
    ...
    it("uses bearer authorization", async () => {
        ...
        await client.request("GET", "/api/health");
        expect(fetcher).toHaveBeenCalledWith("http://localhost:3001/api/health", expect.objectContaining({
            headers: expect.objectContaining({
