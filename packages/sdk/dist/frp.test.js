...
describe("frp sdk", () => {
    it("createAndWait 以 operationJobId 等待并返回 active 映射", async () => {
        ...
        const api = createFrpApi({ request }, jobs);
        await expect(api.createAndWait({
            clientId: "client-1",
            proxyType: "tcp",
            localPort: 1919,
        })).resolves.toMatchObject({ status: "active" });
        expect(jobs.wait).toHaveBeenCalledWith("job-1", expect.any(Object));
        expect(request).toHaveBeenLastCalledWith("GET", "/api/frp/mappings/fm_1", undefined, undefined);
    });
    ...
    it("delete 兼容旧版 AbortSignal 参数", async () => {
        ...
        const api = createFrpApi({ request });
        const controller = new AbortController();
        await api.delete("fm_1", controller.signal);
        expect(request).toHaveBeenCalledWith("DELETE", "/api/frp/mappings/fm_1", undefined, controller.signal);
    });
    it("deleteAndWait 等待删除 Job 完成，不提前声称删除", async () => {
        ...
        expect(request).toHaveBeenCalledWith("DELETE", "/api/frp/mappings/fm_1?timeoutSeconds=45", undefined, undefined);
    ...
    it("create 遇到 busy 时 VcpDeckApiError 原样透传（status 409 / code），不自动重试", async () => {
        const request = vi.fn().mockRejectedValue(new VcpDeckApiError("FRP 映射正在恢复", 409, "FRP_RECONCILE_BUSY"));
        const api = createFrpApi({ request });
        await expect(api.create({ clientId: "client-1", proxyType: "tcp", localPort: 1919 })).rejects.toMatchObject({ status: 409, code: "FRP_RECONCILE_BUSY" });
