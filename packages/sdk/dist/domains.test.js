...
it("calls the Aliyun Drive remote authorization verification endpoint", async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith("/api/aliyundrive/verify", expect.objectContaining({ method: "POST" }));
});
it("reads a safe storage backend summary and switches the backend", async () => {
    ...
    expect(fetcher).toHaveBeenNthCalledWith(1, "/api/storage/config", expect.objectContaining({ method: "GET" }));
    expect(fetcher).toHaveBeenNthCalledWith(2, "/api/storage/config", expect.objectContaining({
        method: "PUT",
