...
describe("jobs", () => {
    it("waits through disconnected and returns done", async () => {
        ...
        const promise = createJobsApi({ request }).wait("j1");
    ...
    it("output 请求输出端点并透传 signal", async () => {
        ...
        const jobs = createJobsApi({ request });
        await expect(jobs.output("j1", controller.signal)).resolves.toEqual({
            jobId: "j1",
            output: "boom\n",
        });
        expect(request).toHaveBeenCalledWith("GET", "/api/jobs/j1/output", undefined, controller.signal);
    });
    it("stops local waiting when aborted", async () => {
        vi.useFakeTimers();
        const controller = new AbortController();
        const promise = createJobsApi({ request: vi.fn() }).wait("j1", {
...
describe("files", () => {
    it("creates and completes an upload session", async () => {
        ...
        expect(request).toHaveBeenNthCalledWith(2, "POST", "/api/files/upload-sessions/j1/complete", { uploadedBytes: 5 }, undefined);
    });
    it("passes overwrite when importing", async () => {
        ...
        await createFilesApi({ request: vi.fn() }, jobs).import("c1", {
            rootDir: "D:\\",
            targetPath: "a.txt",
            fileId: "f1",
            overwrite: true,
    ...
    it("wraps file.roots as a read job", async () => {
        ...
        await expect(createFilesApi({ request: vi.fn() }, jobs).roots("c1")).resolves.toEqual(["C:\\"]);
        expect(jobs.create).toHaveBeenCalledWith({ clientId: "c1", type: "file.roots", payload: {} }, undefined);
    });
});
