...
> 状态：Current｜维护责任：运维/发布维护者｜最后核验：2026-09-19｜适用版本：`0.9.3` / 当前 `main`
...
- **迁移（M1）**：存量 PM2 安装直接重跑同一条安装命令即可（无需 `--migrate`，安装器自动探测存量 PM2 源）；已有 A2 身份且无旧 PM2 候选时按幂等修复处理，不变更身份；若 A2 与有效旧 PM2 同时存在则恢复未完成迁移，以旧身份覆盖误生成的 A2 身份并清理旧 PM2。自动探测范围包括 `/home`、`/var/home` 下用户的 `~/.vcpdeck/launcher-client`、root 的旧安装、ADR-0018 时代的固定目录 `/opt/vcpdeck/launcher-client`，以及 nvm/系统全局安装的 PM2；旧条目可自定义名称，`pm_exec_path` 指向 node、脚本在 `args` 的旧形式也支持。其他 PM2 应用（含 PM2 托管的 VCPDeck Server 与 VCP ToolBox）一律保留；探测不到、查询失败、条目歧义或名称非法时 fail closed。迁移以旧 `launcher.env` 的 `VCPDECK_SERVER` 为权威（Server 归属保留）：从公网入口执行安装命令也会沿用旧 Origin，bootstrap/配置/验收全部走它；旧 Origin 不可达时报 `LINUX_MIGRATION_SERVER_UNREACHABLE` 且不动旧安装。全部材料校验成功后才清理式迁移：核对并删除旧 VCPDeck PM2 entry → `pm2 save` → 无其他 PM2 应用时停用旧自启（否则保留）→ 删除旧 app-dir 与旧安装状态 → 启动 `vcpdeck-client.service` 并全能力验收。**失败只记录阶段状态并保留 A2 现场，可用同一命令重跑，不会 resurrect 或恢复旧 PM2**。迁移前若 `client-id` 非法、PM2 未 online、PM2 entry 指向其他目录、旧 app-dir 不是来源用户的 `.vcpdeck/launcher-client`（或 ADR-0018 固定旧目录）或有进行中 Release，安装器直接拒绝；多用户机器源不唯一时用 `--migrate-from-user=<name>`，需强制重装时用 `--migrate=false`。
...
- Linux 安装报 `检测到未识别的旧 VCPDeck 安装痕迹`：说明旧安装既不在自动探测范围内，也无法作为迁移源。确认旧布局后重跑；若旧条目的 `VCPDECK_SERVER` 与本次 `--server-origin` 不一致（例如 Server 本机用 `http://127.0.0.1:3001`），需用一致地址重跑。仍无法识别时先人工停掉并清理旧 Client（保留其 `client-id` 供复用），或在确认要放弃旧安装、接受全新身份时传 `--migrate=false`；
- Linux 迁移报 `无法确认旧 PM2 状态` 或 `旧 PM2 状态`相关错误：说明该用户的 VCPDeck 应用目录存在，但 `pm2 jlist` 无法读取（PM2 不在私有目录、nvm 或系统全局位置，或输出无法解析）。安装器不会猜，也不会在“看不到其他 PM2 应用”时停用旧自启；请先人工确认 PM2 安装位置后重跑；
...
2. **无显示器机器的显示输出**：Windows 只在存在**活动显示输出**时才有桌面可抓；未接显示器的机器需要 HDMI 假负载（EDID 模拟器）或虚拟显示器驱动（向日葵 / ToDesk / GameViewer 等均会自带）。真无活动输出时前端会显示「未检测到活动显示输出」提示，而不是静默黑屏。
3. **只读建议**：面板**默认只读**（不发送键鼠事件），但只读强度最终取决于客户端行为；需要更强保证时同时在服务端侧配成只读。
4. **平台范围**：当前只承诺 Windows。Linux Client 是 root/systemd 系统级（ADR-0023），不在图形会话内，本期不提供桌面查看。

常见现象对照：

| 现象 | 原因 | 处理 |
| --- | --- | --- |
| 提示「目标端口拒绝连接」 | 目标机没有会话内 VNC 服务端，或不在该端口监听 | 在交互会话内启动 VNC 服务端并仅监听回环 |
| 画面全黑 + 「未检测到活动显示输出」提示 | 目标机无活动显示输出 | 插 HDMI 假负载或安装虚拟显示器驱动后重连 |
| 提示「VNC 认证失败」 | 服务端要求密码但未输入/错误 | 在凭据弹窗输入；或把服务端配成只读无口令 |
| 连接一段时候后断开并出现「第 N 次重连中…」 | 控制面（`/app` socket）掉线，隧道会话被 Server 回收 | 会自动退避重连；持续失败时检查浏览器与 Server 之间的网络/反代超时 |
| 开了「远端跟随」但远端分辨率不变 | 物理 Windows 桌面无法改分辨率（ExtendedDesktopSize 在 Windows 上等同于改物理屏幕） | 预期行为：靠「适配」模式本地等比缩放保证完整显示；需真分辨率跟随请用虚拟显示/X 会话 |
