...
- Linux 安装报 `检测到未识别的旧 VCPDeck 安装痕迹`：旧安装落在自动迁移探测范围外（root 账户、`/opt/vcpdeck/launcher-client`、`~/.nvm` 全局 PM2 等）。安装器不会停掉、也不会删除这类旧安装，且拒绝同时新建身份；请先人工停掉并清理旧 Client（保留其 `client-id` 供复用），再重跑安装命令。确认要放弃旧安装、接受全新身份时传 `--migrate=false`；
