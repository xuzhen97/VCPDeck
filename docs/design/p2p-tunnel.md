...
- 机器「远程桌面」Tab（`desktop-panel.tsx`）：目标端口（默认 5900）+ 连接/断开；一次点击建立一条会话：`create` 会话 → `openBrowserTunnel`，并在其 `onChannel` 回调（DataChannel 创建后、open 之前）内立即 `createVncSession` 挂载 noVNC（否则通道 open 后服务端立即发送的 RFB banner 会被丢弃，握手停在等待版本串），断开 / 卸载 / 切 Tab 都按 `rfb.disconnect()` → `tunnel.close()` → `tunnels.remove()` 固定顺序清理（幂等）。凭据经 `credentialsrequired` 事件弹窗交互（密码只在内存，不落 localStorage）。
  ...
  - **本地缩放（只改浏览器显示，绝不影响目标机）**：提供「适应 / 1:1 / 125% / 150% / 175% / 200%」档位与逐档加减。倍率档位通过**放大 noVNC 挂载容器**实现 —— noVNC 的 `autoscale` 只读容器尺寸、`_rescale` 只设置 canvas 的 `style` 宽高，因此容器变大即画面变大且坐标自洽；超出部分由外层 `overflow-auto` 视口滚动平移。**禁止在 canvas 或其祖先上使用 CSS `transform` / `zoom`**：noVNC 用 `absX(x) = x / display.scale` 换算指针坐标，而 `display.scale` 感知不到外部变换，那样会让指针偏移随离原点距离线性增长（已实测过的缺陷）。
  - 分辨率：**永不发送 `SetDesktopSize`**（`resizeSession` 在 `vnc-session.ts` 内硬编码为 `false`，且会话对象不再暴露 `setResizeSession`，面板不提供远端跟随开关）。在物理 Windows 桌面上 ExtendedDesktopSize 等同于改物理屏幕分辨率，而本产品要求查看行为不得改变目标机显示设置；「完整显示」一律由「适应」模式的本地等比缩放保证。
...
- 本地已验证：① 直连端到端（真实 Server + native Client + 本地 HTTP 目标，DataChannel 直连命中、目标关闭后 `TUNNEL_TARGET_REFUSED`）；② **真实 coturn（Vagrant/VirtualBox Ubuntu VM，`use-auth-secret` + `no-auth`）凭据 A/B** —— 空凭据被拒，Server 签发的 `expiry:sessionId` HMAC-SHA1 凭据被接受并分配 relay 端点，确认与 coturn `use-auth-secret` 完全兼容。③ **真实浏览器（Playwright 驱动的系统 Chrome 153，headful、无代理）矩阵测试**：
  ...
  - **生产环境复测（真实浏览器 + 真实 coturn + 真实 Client）**：强制 relay 时浏览器拿到 `typ relay` 候选且 `ice/connectionState=connected`，面板显示「TURN 中继」。此前失败的两个真实原因已定位并修复：① `scripts/install-coturn.sh` 写出的 `static-auth-secret-file` 不是 coturn 4.6.x 的选项（被判 `Bad configuration format`），coturn 从未加载 shared secret，**所有 TURN 分配恒返回 401**（客户端表现为强制 relay 时零 relay 候选）；② secret 由 `base64`（默认每 76 列换行）写成两行，使 coturn 与 Server 的 HMAC key 不一致。修复方式见 [`deployment.md`](../deployment.md)（systemd drop-in 注入同一份单行 secret）。
- 客户端侧 TURN 凭据写入方式（实现约束，**勿回退**）：`node-datachannel/polyfill` 会把凭据拼成 `turn:<username>:<credential>@host:port` 的 URL，而 coturn REST 的用户名本身含冒号（`<expiry>:<sessionId>`），libdatachannel 解析 userinfo 时在第一个冒号处切开 → 上报给 TURN 的用户名被截断 → `MESSAGE-INTEGRITY` 校验失败（401），客户端自身拿不到 relay 候选。因此 `node-datachannel-peer.ts` 自行构造原生 `PeerConnection`（结构化 `hostname/port/username/password/relayType`，凭据不经过 URL）并以 `peerConnection` 注入 polyfill，保留原有 WebRTC 形状 API；实测客户端 relay 候选由 0 个恢复为 1 个。
