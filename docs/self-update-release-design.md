...
- [ ] F1 完整演练：测试环境走通「上传 → 服务端自更新 → 客户端依次更新 → 手动制造失败验证回退」——集成测试，不适合 TDD（launcher 侧已由 `scripts/smoke-launcher.cjs` 覆盖：正常更新 + 失败自动回退；服务端↔launcher 真实联调待真实环境执行）
- [ ] F2 断线验证：服务端重启窗口内完成的 job 结果不丢（socket.io 缓冲 + 重连核对）——集成测试，不适合 TDD（待真实环境执行）
