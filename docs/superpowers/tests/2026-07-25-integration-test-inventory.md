...
>
> 自动启动 Server 进程 + 连接 mock client + 启动真实 Client → 执行 47 个用例 → 清理退出。
...
| — | Enabled identity can login again | 启用后可重新登录 `200/201` |
...
| # | 名称 | 验证点 |
|---|------|--------|
| 37 | exec command legacy | 旧 `{command}` payload → exitCode=0（未使用 waitForJobUpdate） |
| 38 | exec command explicit | 显式 `{mode:"command",command}` → exitCode=0 |
| 39 | exec command cwd | 带 `cwd` 的命令 → exitCode=0 |

**Mock/Real:** **真实 Client 进程** spawn(`echo`) → `JOB_UPDATE` 广播 → monitor socket 校验
...
| 43 | exec script empty args/script | `args:[]` + `script:""` → 不崩溃，正常终态 |

**Mock/Real:** **真实 Client 进程** spawn(`process.execPath`, `["-"]`) → stdin 写入 → `JOB_UPDATE` 校验

---

## 12. Exec 错误与取消 —— 真实 Client（3 项）

| # | 名称 | 验证点 |
|---|------|--------|
| 44 | exec spawn failed | 不存在的 executable → `EXEC_SPAWN_FAILED` |
| 45 | exec cancel | 长时间运行脚本 → cancel 请求 → `cancelled` 终态 |

**Mock/Real:** **真实 Client 进程** — spawn 错误由真实 executor 产生，cancel 经由 real client 的 `killJob` 处理
...
| # | 名称 | 验证点 |
|---|------|--------|
| 46 | Logout | POST /api/auth/logout 成功 |
| 47 | Session invalid after logout | 退出后 /api/auth/me 返回 `401` |
...
3. 使用预置工具函数：
   ...
   - `sleep(ms)` — 异步等待
   - `waitForJobUpdate(jobId)` — 监听真实 Client 的 Job 完成
...
- 测试名称在 `pass/fail` 的第一个参数中，建议按模块前缀（如 `exec command legacy`, `exec script node`）
...
| Job | timeout 触发 | 需要长时间等待，可用短 timeout |
| Exec script | Python/PowerShell/Bash 运行时 | 需要对应解释器安装在 CI 环境 |
| Exec script | 大脚本（接近传输上限） | 当前无应用层上限 |
| Exec script | `cwd` 不存在 | 应报 `EXEC_SPAWN_FAILED` |
| Exec | 并发超过 MAX_CONCURRENT_JOBS | 当前排队但未独立验证排队行为 |
