...
- **Pi 思考过程现在可见（实时流式 + 历史按需）**：
  - **实时**：`thinking_progress` 的 delta 本来就在流（单次 ≤ 16 KiB），现在当前回合的思考块**默认展开并随 delta 实时追加**（不再先要求点一次「展开思考」）；收到 `thinking_end` 后折叠为「已思考 N 秒」摘要，结算后保留（不再瞬间消失），用户随时可再展开。
  - **历史**：先前展开历史消息里的思考块只会看到红字「思考内容不可用」——历史投影只带 deferred 占位、正文不随分页下发，而 pi-web 渲染层默认去请求它自己的本地接口（VCPDeck 里不存在该路径）。现在由 Pi UI Adapter 注入取数实现，展开时经 `session.entryContent` 按需取回一次（与历史图片同一寻址/惰性模式，带缓存、失败不落缓存）；未注入或失败时仍旧显示不可用提示，**不伪造内容**。
  - **边界**：thinking 正文不落盘到 Server、不进 Job payload/result、不进日志与审计；实时正文只留在本轮内存状态里。vendored 渲染层的此次改动以 `scripts/port-pi-web-render.mjs` 的声明式补丁规则实现（重跑可重现、锚点缺失即失败），已在 ADR-0032 记为例外条款。
...
- **Pi 时间线：思考块与 Process Details 的位置错误**：实时思考块原本渲染在所有回合组之前（时间线顶部的一次全局渲染），空闲后还会永久残留、跨轮甚至跨会话保留；首条 user 消息之前的非 user 消息会被开成孤立组，让 `Process Details` 渲染到提问气泡**上方**。现在实时思考块位于本轮回合内部（提问气泡之后、Process Details 之前）并在流式期间默认展开；前置残段并入随后的第一个回合；结算后保留「已思考 N 秒」摘要。
