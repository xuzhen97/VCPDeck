...
async function main() {
  ...
  let adminIdentity = null;
  {
    ...
    if ((status === 200 || status === 201) && body.identity?.isAdmin && body.identity?.username === "admin") {
      adminIdentity = body.identity;
      pass("Login as admin", `id=${body.identity.id.slice(0, 8)}..., isAdmin=true`);
    } else {
  ...
  let tokenId = null;
  {
    ...
    if ((status === 200 || status === 201) && body.token?.startsWith("vcp_") && body.label === "test-integration-cli") {
      cliToken = body.token;
      tokenId = body.id;
  ...
  console.log("\n--- Auth: bearer token ---");
  if (cliToken) {
    const res = await api("GET", "/api/clients", { bearer: cliToken, noCookie: true });
    const body = await res.json();
  ...
  const ts = Date.now();
  {
    const { status, body } = await apiJson("POST", "/api/identities", {
      json: { username: `user-${ts}`, password: "pass123", displayName: "Integration Test User" },
    });
    if ((status === 200 || status === 201) && body.username === `user-${ts}` && body.isAdmin === false) {
      user1Id = body.id;
  ...
  {
    const { status } = await apiJson("POST", `/api/identities/${user1Id}/disable`);
  ...
  {
    const { status } = await apiJson("POST", `/api/identities/${user1Id}/enable`);
    if (status === 200 || status === 201) {
  ...
  // 18. Enabled user can login again
  {
    ...
    if ((status === 200 || status === 201) && body.identity?.username === userLoginName) {
      pass("Enabled identity can login again", `${status}`);
    } else {
      fail("Enabled identity can login again", JSON.stringify({ status, body }));
    }
  }
    ...
    try {
      ...
      if (dispatchReceived) {
        ...
        if (ourJob && ourJob.status === "done") {
          ...
          if (ourJob.createdByIdentityId === adminIdentity?.id && ourJob.createdByName === "admin" && ourJob.createdVia === "web") {
            pass("Job has auth audit fields", `by=${ourJob.createdByName}, via=${ourJob.createdVia}`);
          } else {
