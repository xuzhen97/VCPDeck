 ...
 *
 * 测试点（14 项）：
 *   1. GET /api/clients         — 空状态返回数组
 *   2. GET /api/jobs             — 空状态返回数组
 *   3. Socket.IO 连接            — client 连上 server
...
async function main() {
	...
	if (clientSocket?.connected && clientId) {
		try {
			const realClientId = (await fetchJson(`${BASE}/api/clients`))[0]
				?.clientId;
	...
	if (clientSocket?.connected && clientId) {
		try {
			...
			const cancelResult = await fetchJson(
				`${BASE}/api/jobs/${job.jobId}/cancel`,
				{
					method: "POST",
				},
			);
