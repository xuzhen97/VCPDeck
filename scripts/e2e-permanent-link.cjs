...
async function main() {
	...
	if (login.status !== 201 && login.status !== 200)
		return fail("登录", `HTTP ${login.status}`);
	pass("登录");
	...
	if (token.status !== 201 && token.status !== 200)
		return fail("上传令牌", `HTTP ${token.status}`);
	...
	const putRes = await fetch(`${BASE}${uploadPath}`, {
		method: "PUT",
		body: content,
	});
	...
	// 4. 签永久下载令牌
	const dlToken = await api("POST", "/api/storage/download-token", {
		key: uploaded.key,
		ttlSeconds: 0,
	});
	if (dlToken.status !== 201 && dlToken.status !== 200)
		return fail("永久下载令牌", `HTTP ${dlToken.status}`);
	const dlUrl = dlToken.body.url;
	if (!dlUrl.includes("expires=0"))
		return fail("永久下载令牌", `url 缺 expires=0: ${dlUrl}`);
	...
	const tmpToken = await api("POST", "/api/storage/download-token", {
		key: uploaded.key,
	});
	if (
		!tmpToken.body.url.includes("expires=") ||
		tmpToken.body.url.includes("expires=0")
	) {
